...
export function useAudioPrefs() {
  ...
  useEffect(() => { autoExportRef.current = autoExportLikes; }, [autoExportLikes]);
  useEffect(() => { genreRef.current = genreFilters; }, [genreFilters]);
  useEffect(() => { languageRef.current = languageFilters; }, [languageFilters]);
  ...
  const setMuted = useCallback(async (next: boolean) => {
    setMutedState(next);
    try { setBool(KEY_MUTED, next); } catch {}
  ...
  const setGenreFilters = useCallback(async (next: string[]) => {
    ...
    try { setString(KEY_GENRES, JSON.stringify(next)); } catch {}
  ...
  const setLanguageFilters = useCallback(async (next: string[]) => {
    ...
    try { setString(KEY_LANGUAGES, JSON.stringify(next)); } catch {}
